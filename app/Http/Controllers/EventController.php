...
$Users = DB::table('Events')
            ->select('image','Reference','E_title','Type','Categorie','price','bedroom','bathroom','Street_Address','City','State','sqft','Furnishing','build_year','about','postor','updated_at','Verified')
            ->get();
...
$Users = DB::table('feedbacks')
            ->select('Reference','Type','Sender','Email','HouseRef','Message','updated_at','status','Owner','Tenants')
            ->where('User_Ref',$Num)    
            ->get();
...
$mail->addAddress("suleimanhashi96@gmail.com", "Abdifitah Aden Nur Abdi");

$mail->isHTML(true);

$mail->Subject = "You have received a new feedback from ".$request->input('name');
$mail->Body = "

Dear Abdifitah Aden Nur Abdi<br><br><br>

<b>Name : </b>".$request->input('name')."<br>
<b>Email : </b>".$request->input('Email')."<br>
<b>Message : </b>".$request->input('Message')."<br>

<br><br> Real State Portal team";

$mail->AltBody = "Real State Portal team";
...
$Users = DB::table('bills')
            ->select('Reference','HouseRef','Tenants','Email','status','created_at','price') 
            ->where('Email',$Num)
            ->get();
...
$HouseRef = $request->input('HouseRef');

$Users = DB::table('events')
            ->select('*')
            ->join('users', 'users.Reference', '=', 'events.postor')
            ->where('events.Reference',$HouseRef)
            ->get();



return view('Agreement',['events'=>$Users,'Name'=>$request->input('Sender')]);
...
public function billsInvoice(Request $request){


return view('billsView',['Reference'=>$request->input('id'),'HouseRef'=>$request->input('HouseRef'),'Tenants'=>$request->input('Tenants'),'Email'=>$request->input('Email'),'status'=>$request->input('status'),'price'=>$request->input('price')]);
...
$events = DB::table('Events')
            ->select('image','Reference','E_title','Type','Categorie','price','bedroom','bathroom','Street_Address','City','State','sqft','Furnishing','build_year','about','updated_at','Verified')
            ->get();
return view('Main_Events',['events'=>$events]);}
}
...
$P = DB::table('events')
            ->select('image','Reference','E_title','Type','Categorie','price','bedroom','bathroom','Street_Address','City','State','sqft','Furnishing','build_year','postor','about','Verified','updated_at')
...
$query = Feedbacks::where('Reference', $ref)
       ->update([
"status" => "Approved",
  ]);
 ...
 if ($query > 0) {
     echo "successfully Send the Agreement to Tenants";
 } else {
       echo "could not be send, please try again!";
...
$users = DB::table('events')
            ->select('price')
            ->where('Reference',$request->input('HouseRef'))
            ->get();
...
bills::create([
 ...
 if ($query > 0) {
     echo "successfully Signed the Agreement and the first month bill is generated";
 } else {
       echo "could not be sign, please try again!";
...
protected function SignOwner(Request $request){
...
"Owner" => "Signed",
 ...
 if ($query > 0) {
     echo "successfully Signed the Agreement";
 } else {
       echo "could not be sign, please try again!";
...
$query = bills::where('Reference', $ref)
       ->update([
"status" => "Paid",
  ]);
...
$query = events::where('Reference', $ref)
       ->update([
"Verified" => "Verified",
  ]);
 ...
 if ($query > 0) {
     echo "successfully Verified";
       ...
       echo "could not be Verified, please try again!";
